...
import grblmsk_post
grblmsk_post.export(object, "/path/to/file.ncc")
"""
...
OUTPUT_TOOL_CHANGE = (
    True  # default don't output M6 tool changes (comment it) as grbl currently does not handle it
)
DRILL_RETRACT_MODE = (
...
parser = argparse.ArgumentParser(prog="grblmsk", add_help=False)
...
def parse(pathobj):
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        # Look for the first (X,Y) location, so we can rapid to it at
        # max Z height *before* we do the Z-down move.
        # MSK 2026-02-09
        got_startxy = False  

        for c in PathUtils.getPathWithPlacement(pathobj).Commands:

            # MSK additions 2026-02-09: The default behavior for
            # this post is to rapid down to the 'clearance' Z-height
            # immediately after the toolchange/spindle up. I feel
            # much better, however, if we delay that Z move until
            # after we're over the cutting start point. This way, since
            # we generally keep all the cutting action 'inside' the 
            # clamps, we can move around freely without having to go
            # too high when moving between features.
            if c.Name == 'G0' and got_startxy is False:
                startx, starty = None, None
                for gc in PathUtils.getPathWithPlacement(pathobj).Commands:
                    if 'X' in gc.Parameters:
                        startx = gc.Parameters['X']
                    if 'Y' in gc.Parameters:
                        starty = gc.Parameters['Y']
                    if startx is not None and starty is not None:
                        out += f"G0 X{startx:.3f} Y{starty:.3f}\n"
                        got_startxy = True
                        break
